...
function (angular, app, _, require, kbn) {
  ...
  module.controller('pcap', function($scope, $http, $rootScope, $modal, $q, $compile, $timeout, fields, querySrv, filterSrv, dashboard) {
    ...
    $scope.get_data = function(dashboard, segment, query_id) {
      ...
      var sp_hex = source_port.toString(16);
      console.log(sp_hex)

      // Get the pcap identifiers
      $scope.pcap_query = [source_port, dest_port, source_ip, dest_ip, protocol];
      // console.log($scope.pcap_query);
      ...
      $scope.panel.queries.identifiers = querySrv.idsByMode($scope.panel.queries); // query goes here

      queries = querySrv.getQueryObjs($scope.panel.queries.identifiers);
