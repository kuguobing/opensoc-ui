...
function (angular, $, kbn, _, config, moment, Modernizr) {
  ...
  module.service('dashboard', function(
    ...
    // load pcap
    this.load_pcap = function(dashboard) { // takes pcap parameter
      console.log(dashboard)
      if(self.current.pcap) {
        console.log('turnt up!');
        dashboard.current.rows[1].panels.push({ //shift this
          title: 'New Test PCAP',
          type: 'pcap'
        });
      } else {
        console.log('no pcap panel')
      }
    }
