  ...
  module.service('querySrv', function(dashboard, ejsResource, filterSrv, esVersion, $q) {
    ...
    this.queryTypes = {
      ...
      parsed: {
        ...
        icon: "icon-exchange",
        resolve: function (query) {
          var ejsSuccess = function (results) {
            console.log("Results:");
            console.log(results);
            return results;
          };

          // resolve against node query parser proxy
          var p = jQuery.ajax('/parse?query=' + query.query).
            then(function (data) {
              console.log("Parsed: ");
              console.log(JSON.parse(data));
              return _.extend(query, { query: JSON.parse(data), parent: query.id });

              // ejs.client.post('_search', JSON.stringify(data)).
              //   then(ejsSuccess, function (err) { console.error('error'); });
            });
